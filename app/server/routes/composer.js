...
exports.post = function get(req, res) {
	...
	if (req.body.to && (req.body.message_it || req.body.message_en)) {
		...
		for(var a=0;a<items.length;a++){
			...
			rows[i].message = req.body["message_"+items[a][0]].replace('[org_name]',items[a][1]).replace('[name]',items[a][2]).replace('[id]',items[a][8]).replace('[login]',items[a][9]).replace('[signature]',req.body.from_name)+signature;
			rows[i].server = {
